...
public class VentanaRegistrarEstudiante extends Ventana{
    ...
    private boolean agregarEstudiante() throws ClassNotFoundException {
        if(this.campoMatricula.getText().length()==0 || this.campoNombre.getText().length()==0 || this.campoRut.getText().length()==0 || this.campoCarrera.getSelectedItem().equals("Error no existen carreras")){
            ...
            return EstudianteController.agregarEstudiante(this.campoNombre.getText(),this.campoRut.getText(), this.campoMatricula.getText(),this.campoCarrera.getSelectedItem().toString());
        }
    }
